<?php
/*
    Kalender mit Kalenderwochen generieren  (Docker Version)
...
function startup()
{
    global $docker_call, $docker_vz, $pgm_dat; 
    $data = $docker_vz."/".$pgm_dat;
    $os_bef = "";
    if (file_exists($docker_vz))
    {
        //chmod($docker_vz, "0666");
        $os_bef = "sudo chmod 777 ".$docker_vz;
        shell_exec($os_bef);
        //chown($docker_vz, "www-data");
        $os_bef = "sudo chown www-data:www-data ".$docker_vz;
        shell_exec($os_bef);        
        //chmod($pgm_dat, "0666");
        $os_bef = "sudo chmod 666 ".$pgm_dat;
        shell_exec($os_bef);        
        //chown($pgm_dat, "www-data");
        $os_bef = "sudo chown www-data:www-data ".$pgm_dat;
        shell_exec($os_bef);         
        
        $docker_call = 1;
        if (!file_exists($data))
        {
            //$erg = copy($pgm_dat, $data);
            $os_bef = "sudo cp ".$pgm_dat." ".$data;
            shell_exec($os_bef);
    ...
    <html lang="de">
        ...
        <body>
            ...
            for ($inda = 1; $inda <= 12; $inda++)
            {
                ...
                if ($inda == 1) echo ('<a href="jkalender.php?jahr='.($zlr_jahr-1).'">');
                if ($inda == 12) echo ('<a href="jkalender.php?jahr='.($zlr_jahr+1).'">');                
                echo ("<table>"."\r\n");
